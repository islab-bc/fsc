...
func Update(ctx contractapi.TransactionContextInterface, nur request.NoticeUpdateRequest) error {
	...
	if len(INFCreated) == 0 { // fixed만 존재할 경우 기존값 삭제 후 not-fixed로 업데이트
	...
	if len(IFCreated) == 0 { // not-fixed만 존재하는 경우 기존값에 덮어쓰기
...
func overwriteOnly(ctx contractapi.TransactionContextInterface, targetNoticeModel *model.Notice) error {
	...
	err = ctx.GetStub().PutState(targetNoticeModel.GetINFKey(), targetNoticeModelAsBytes) // overwrite with INFKey
	if err != nil {
		logger.Error(err.Error())
...
func updateWithDelete(ctx contractapi.TransactionContextInterface, targetNoticeModel *model.Notice) error {
	...
	err = ctx.GetStub().DelState(targetNoticeModel.GetIFKey()) // delete with IF Key
	if err != nil {
		logger.Error(err.Error())
		return errors.New("DelState() 에러 발생")
	}
	err = ctx.GetStub().PutState(targetNoticeModel.GetINFKey(), targetNoticeModelAsBytes) // put with INF Key
	if err != nil {
		logger.Error(err.Error())
		return errors.New("PutState 실패")
	}
